...
(function () {
  ...
  function build() {
    ...
    var BLADES_NEAR = small ? 45000 : 75000;
    var BLADES_FAR  = small ? 15000 : 22000;

    var q = /[?&]blades=(\d+)/.exec(location.search);
    if (q) { BLADES_NEAR = +q[1]; BLADES_FAR = Math.round(+q[1] * 0.21); }

    renderer = new THREE.WebGLRenderer({ canvas: canvas, alpha: true, antialias: false, powerPreference: 'high-performance', precision: 'mediump' });
  ...
  /* Kick the entrance off first. Growing the roots and compiling the
     ...
     frame — so hand the browser two frames to start animating. */
            ...
            /* ── Habitat interactive bridge ──────────────────────────────────── */
            function dispatchHabitatNav(path) {
